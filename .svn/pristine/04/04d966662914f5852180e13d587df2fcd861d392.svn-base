...
function(a, b, c) {
    ...
    function f(b) {
        ...
        b, !0),
        c.option.selector && c.option.data && (c.ele = a(c.option.selector), c.option.data && c.set(c.option.data, c.option.index))
    }
    f.prototype = {
        focusAnim: function(b, c) {
            ...
            if (d.option.hasAnim) {
                ...
                d.focus = d.focus || a('<div class="focus utra bc-head"></div>'),
                d.focus.css("width", f),
                d.focus.css("-webkit-transform", "translateX(" + b * f + "px)"),
                c && c.append(d.focus)
            }
        },
        focusText: function(b) {
            ...
            if (! (c.option.hasAnim && c.option.hasIcon && c.option.hasLabel)) {
                ...
                d.length && (d.removeClass("bac"), a(d[b]).addClass("bac"))
            }
        },
        set: function(b, c) {
            var d = this;
            d.itemCount = b.length,
            d.focus && (d.focus = null),
            d.ele.empty();
            var f = a('<div class="uf t-bla ub" ></div>');
            d.option.hasIcon && d.option.hasLabel ? f.addClass("tab_m") : d.option.hasLabel ? f.addClass("tab_l") : d.option.hasIcon && f.addClass("tab_i"),
            d.focusAnim(c, f);
            for (var g in b) {
                var h = a(e({
                    data: b[g],
