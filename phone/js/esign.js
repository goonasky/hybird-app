...
(function (root, doc, factory) {
    ...
    $.fn.esign = function (canvasEdit, sign_show, sign_clear, sign_ok) {
        ...
        function convertCanvasToImage(canvas) {
            ...
            image.base64 = image.src.split("base64,")[1];
            //===================================================================
            $(".pwd-box").removeClass("none");
            var len
            var pwd
            var url1 = http1 + "Qianzi";
            var url2 = http + "CompareHousePassWord";
            var info = image.base64;
            $(".check_password").off().on("click", function () {
                pwd = $("#pwd1").val();
                len = pwd.length;
                //alert(len);
                if (len == 6) {
                    if (pwd == "123456") {
                        $.ajax({
                            ...
                            success: function (data) {
                                var picName = data.userdata.record[0].Name;
                                var db = uexDataBaseMgr.open(dbName);
                                if (!db) { } else {
                                    var sqls = [

                                        "Update MaintainRecord  set Status=1,sign='" + picName + "' Where GUID in(" + localStorage.tempguid + ")"
                                    ]
                                    uexDataBaseMgr.transactionEx(db, JSON.stringify(sqls),
                                        function (error) {
                                            uexWindow.toast("0", "5", "签字成功", "2000");
                                            $("#pwd1").val("");
                                            $(".pwd-box").addClass("none");
                                            uexWindow.open('MtSign', '0', "MtSign_top.html", '10', '', '', '4');
                                        });
                                }
